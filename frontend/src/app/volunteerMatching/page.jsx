  ...
  return (
    <>
      <div className="max-h-[calc(100vh-12rem)] h-[calc(100vh-12rem)] mt-20">
        <CardGrid events={events} title="Event Matching" buttonLabel="Match Volunteers" tooltip={true} onEventClick={handleEditEvent} titleAction={
      ...
      {isModalOpen && (
        <div className="fixed inset-0 backdrop-blur-xs flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded shadow-lg w-full max-w-md relative border border-black">
            <button onClick={() => {setIsModalOpen(false); setSelectedEvent(null); setNewEvent({ title: '', date: '', urgency: '', description: '', image: '' });}} 
            ...
            <div className="flex flex-col space-y-3">
              ...
              <div className="flex items-center">
                ...
                <input type="text" id="eventDate" placeholder="MM/DD/YY" className="border px-3 py-2 rounded w-full" value={selectedEvent ? selectedEvent.date : newEvent.date}
                  onChange={(e) => {
                    const value = e.target.value;
                    if (selectedEvent) {
              ...
              <div className="flex justify-between items-center mt-4">
                {selectedEvent ? (
                  <button onClick={() => {setEvents(events.filter(ev => ev.id !== selectedEvent.id)); setIsModalOpen(false);
                      setSelectedEvent(null); setNewEvent({ title: '', date: '', urgency: '', description: '', image: '' });
